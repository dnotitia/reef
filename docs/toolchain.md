...
`@types/node`, `tsx`, `typescript`, `vitest`, and `zod`. The repository's
TypeScript baseline is 6.0.2, declared as `^6.0.2` in the catalog and normally
consumed through `catalog:` by workspaces that run the compiler. Workspace
manifests reference those versions with `catalog:` except for the Git-prepared
package rule below. Internal package edges
...
the workspace, so it repeats the catalog's concrete ranges and the exact root
build-tool version it needs. The policy checker rejects `catalog:` in such a
manifest and rejects any repeated range that drifts from the root source of
truth. `@reef/core` is currently the only package with this contract.
...
unapproved duplicate root tools, Git-prepared package drift, invalid workspace
