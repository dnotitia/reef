...
4. **Issue body links use Markdown; issue relationships use fields.** In generated issue body prose, write external links as standard Markdown (\`[label](https://...)\`) with a human-readable label and an absolute URL. Mention Reef issues as plain ids (\`REEF-123\`) unless you are setting \`depends_on\`, \`blocks\`, \`related_to\`, or \`parent_id\`, which live in document fields and the \`reef_issues\` row. Do not hand-write wiki links or synthetic \`akb://\` paths in the body. (issue-workflows.md)

5. **Planning links are a convention to honor, not an afterthought.** When the workspace shares a milestone/release or hangs issues under an epic, propose matching parent_id / milestone_id / release_id and confirm -- never leave a new issue all-null, never auto-write them silently. Why: a board where every issue shares one milestone/release but the newest one is unlinked reads as a mistake. A sprint is the exception: it is a commitment, so never attach one to a backlog issue -- a new issue going into the current sprint is created as todo, not backlog. (conversational-playbook.md + planning-workflows.md)
