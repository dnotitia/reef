...
4. Seed the issue body from a matching template. Template names vary per vault, so do not assume the issue_type is a template name -- list what exists and pick the closest fit:
   ...
   When filling the body, use standard Markdown for inline links: \`[label](https://example.com/path)\` with a human-readable label and an absolute external URL. Mention Reef issues in prose as plain ids (\`REEF-012\`), then set \`depends_on\`, \`blocks\`, \`related_to\`, or \`parent_id\` fields when there is an actual issue relationship. Do not use wiki-style links (\`[[REEF-012]]\`), raw HTML anchors, or hand-built \`akb://\` paths in issue bodies; if an AKB resource must be referenced, use the exact URI returned by an AKB tool, and prefer the first-class relation flow when the target is another AKB document. Structured PM-facing links belong in \`meta.external_refs\`, and delivery evidence belongs in \`meta.implementation_refs\`, instead of being only body prose.
