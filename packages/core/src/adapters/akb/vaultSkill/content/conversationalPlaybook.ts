...
requester records who asked for the work, separate from assigned_to (who will do it) and distinct in purpose from meta.author. meta.author is the resolved actor that created the row, and the created_by audit field projects from it; it is not your own agent identity. On a conversational create, default requester to the acting user resolved with akb_whoami: the person making the request is the one asking for it. If the user explicitly names someone else as the requester ("file this on behalf of Dana"), use that person instead. This is the one inferred field you need not surface in the create proposal (the "propose, then write" rule): the requester is almost always the person speaking, so confirming it would be noise -- just set it, and name it in the read-back if it helps.
