...
Each row carries body (the comment's markdown text) and meta with author, created_at, and edited_at. As with activity, the author and timestamps are projected from meta -- NOT from akb's auto created_by/created_at columns. A null edited_at means the comment was never edited; a non-null edited_at marks an edited comment.
...
2. Guard against a non-existent issue. Confirm the issue's row exists before inserting -- reef ids are predictable, so an orphan comment row would otherwise resurface on a future issue that reuses the id:

   SELECT reef_id FROM reef_issues WHERE reef_id = 'REEF-001' LIMIT 1

   If this returns no row, stop and tell the user the issue does not exist; do not insert.
3. Insert the comment. body is the markdown text; meta carries the author, the ISO-8601 created_at, and edited_at = null on a fresh comment:

   INSERT INTO reef_comments (reef_id, body, meta)
   VALUES (
     'REEF-001',
     'Looks good -- shipping once review passes.',
     '{"author":"ACTOR","created_at":"2026-06-19T07:34:38.237Z","edited_at":null}'::json);
...
- Do NOT set id, created_by, created_at, or updated_at columns; AKB fills them. reef's canonical author and write time live in meta, not those akb columns.
