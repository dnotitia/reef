...
and LLM configuration is deployment-managed server environment.
...
- Expose Route Handlers under `src/app/api/*` that validate inputs, extract
  ...
  responses.
- Manage browser-local UI and credential state through Dexie, localStorage, and
  TanStack Query persistence where appropriate.
...
| `src/lib/` | API client helpers, AKB/session helpers, logging, metrics, telemetry, LLM config, GitHub token extraction, and browser storage helpers. |
...
- Dexie IndexedDB stores browser-local `config` and `credentials`.
- `credentials` stores the GitHub PAT only.
...
- Extract AKB credentials from the cookie and GitHub credentials from request
  headers.
