...
- `config` holds active `vault`, theme, locale, `akb_user_id`, and per-vault UI
  preferences such as saved issue filters. Locale is mirrored to the
  `NEXT_LOCALE` cookie for SSR,
