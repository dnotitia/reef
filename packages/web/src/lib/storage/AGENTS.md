...
- `config` holds active `vault`, theme, locale, `akb_user_id`, activity visit
  ...
  saved issue filters, the default saved-view pointer, and explicit Favorite
  saved-view ids. Locale is mirrored to the `NEXT_LOCALE` cookie for SSR, while
  IndexedDB remains canonical. The store does not hold monitored repos,
