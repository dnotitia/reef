...
export async function POST(
  ...
  if (!parsed.success) {
    return jsonAgentError(
      "Artifact dismissal request is missing or invalid.",
      400,
  ...
  if (!parsed.data.artifact) {
    return jsonAgentError(
      "Artifact dismissal requires an artifact payload.",
      400,
      "missing_artifact",
      { artifact_id: id },
    );
  ...
  if (
    ...
    return jsonAgentError(
      "Artifact dismissal requires a vault for persisted activity suggestions.",
      400,
      "missing_vault",
      { artifact_id: id },
    );
