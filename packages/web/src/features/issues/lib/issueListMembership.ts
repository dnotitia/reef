 ...
 * logs (assignee / priority / planning links / impl refs, REEF-126; the
 * REEF-277 parity set: title / due date / estimate / parent / archive / labels /
 * relations; and issue type / start date). Editing any of them should refetch
 * the issue's activity query so the unified timeline shows the freshly logged
 * event immediately, the same path status changes already used (REEF-064) —
 * not just `status`. Keep this list in lockstep with `diffFieldActivityEvents`:
 * a logged field missing here leaves its event invisible until a stale-window
 * refetch or full reload.
 */
