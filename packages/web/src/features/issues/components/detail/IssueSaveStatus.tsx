...
function IssueSaveStatusComponent({
  ...
  if (status === "saved") {
    return (
      <span
        data-testid="issue-save-status"
        className="inline-flex items-center gap-1 type-card-metadata text-muted-foreground"
      >
  ...
  if (status === "error") {
    return (
      <span
        data-testid="issue-save-status"
        className="inline-flex items-center gap-1 type-card-metadata text-destructive-text"
      >
