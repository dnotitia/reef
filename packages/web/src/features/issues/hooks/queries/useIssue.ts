 ...
 * caches so the same issue id in two workspaces does not share one entry (which
 * would let one workspace's data render — and autosave — under another).
 * Disabled when either input is empty.
 *
 * `refetchOnMount: "consistently"` (REEF-227): opening the issue card consistently
 ...
 * detail query (its consumer is the detail card); the list/global
