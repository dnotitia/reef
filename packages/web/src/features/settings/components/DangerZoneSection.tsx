...
/**
 * Owner-only workspace-lifecycle danger zone at the foot of Settings › Workspace
 * (REEF-322). Two destructive actions whose blast radius is encoded in the
 * button weight: detach (outline) removes only the reef layer and keeps the akb
 ...
 * over-promises. Non-owners (and readers/writers/admins) never see it.
 */
export function DangerZoneSection({ vault }: DangerZoneSectionProps) {
  ...
  // Omit until the role resolves so a wrong gate never flashes, and only for the
  ...
  return (
    <section className="flex flex-col gap-3" data-testid="danger-zone-section">
      ...
      <WorkspaceDestructiveDialog
        // Remount per opened action so the type-to-confirm field never carries
