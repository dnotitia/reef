...
export function WorkspaceSkillSection() {
  ...
  const tm = useTranslations("settings.misc");
  ...
  if (status.isError || !status.data) {
    return (
      <p className="text-sm text-muted-foreground">{tm("skillStatusError")}</p>
    );
  }
  ...
  return (
    <div className="rounded-md border border-status-in-progress/40 bg-status-in-progress/5 px-3 py-3 text-sm">
      ...
      <p className="mt-1 text-muted-foreground">{tm("updateAvailableBody")}</p>
      ...
      <div className="mt-3">
        {!canWrite ? (
          <p className="text-muted-foreground">{tm("needsWriteAccess")}</p>
        ) : confirming ? (
          <div className="flex flex-col gap-2">
            <p className="text-muted-foreground">{tm("confirmReplace")}</p>
