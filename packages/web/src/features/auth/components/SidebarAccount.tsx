...
import { ChevronsUpDown, ExternalLink, Keyboard, LogOut } from "lucide-react";
 ...
 * Shown regardless of GitHub PAT state (AC1). The shortcuts launcher is now a
 * separate footer utility row above the account row; the menu keeps version context
 * as a release-notes link (REEF-170).
 */
export function SidebarAccount({ appVersion, collapsed }: SidebarAccountProps) {
  ...
  const shortcutsButton = (
    <button
      ...
      className={cn(
        "inline-flex shrink-0 items-center rounded-md text-muted-foreground transition-colors hover:bg-surface-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40",
        collapsed
          ? "h-9 w-9 justify-center"
          : "h-8 w-full justify-between gap-2 px-2 text-left",
      )}
    >
      <span className={cn("flex items-center", !collapsed && "gap-2")}>
        <Keyboard aria-hidden="true" className="size-4" />
        {!collapsed && (
          <span className="truncate text-[13px]">Keyboard shortcuts</span>
        )}
      </span>
      {!collapsed && (
        <span
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex",
          collapsed ? "flex-col items-center gap-1" : "flex-col gap-1",
        )}
        data-testid="sidebar-account-actions"
      >
        {shortcutsButton}
