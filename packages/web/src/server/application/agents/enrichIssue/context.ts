...
export async function resolveVerifiedRepoContext({
  ...
  let configResult: Awaited<ReturnType<typeof akbReadConfig>>;
  try {
    configResult = await akbReadConfig({ adapter: akbAdapter, vault });
  } catch (err) {
    const boundaryError = toWorkspaceBoundaryError(err);
    if (boundaryError) {
      throw boundaryError;
...
export async function buildEnrichmentContext({
  ...
  const [issuesResult, templateEntries, planningCatalog] = await Promise.all([
    safeContextPart(
      span,
      "issues",
      () => akbListIssues({ adapter: akbAdapter, vault }),
      { issues: [] },
    ),
    safeContextPart(
      span,
      "templates",
      () => akbListTemplates({ adapter: akbAdapter, vault }),
      [],
    ),
    safeContextPart(
      span,
      "planning",
      () => akbListPlanningCatalog({ adapter: akbAdapter, vault }),
      undefined,
    ),
  ]);
