...
describe("createSearchIssuesTool", () => {
  ...
  it("memoises empty-query akbListIssues across calls within the same tool instance", async () => {
  ...
  it("re-fetches on the next call when the first akbListIssues round throws", async () => {
    // First akbListIssues SELECT returns 401; the cache is reset by the catch
