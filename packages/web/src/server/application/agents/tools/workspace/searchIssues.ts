 ...
 *
 * To avoid hammering akb for empty-query calls, `akbListIssues` is memoised in the
...
export function createSearchIssuesTool({
  ...
  function loadAllIssues(): Promise<readonly IssueMetadata[]> {
    ...
    cachedListPromise = akbListIssues({ adapter, vault })
...
async function hybridSearchIssues({
  ...
  const hits = await akbSearchDocuments({
    adapter,
    vault,
    collection: "issues",
    type: "task",
