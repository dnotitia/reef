import { akbBuildReefVaultSkillDocuments } from "@reef/core";
import { EnrichmentFieldEnum } from "@reef/core";
import { ExternalRefTypeEnum, IssueCreateFieldsSchema } from "@reef/core";
...
describe("issue field drift guards", () => {
  ...
  it("keeps vault-skill external reference guidance aligned with schema", () => {
    const content = akbBuildReefVaultSkillDocuments("reef")
      .map((doc) => doc.content)
      .join("\n");
