...
import { ISSUE_ID_PATTERN } from "@reef/core";
import {
...
async function resolveMonitoredRepos({
  ...
  try {
    const { config } = await akbReadConfig({ adapter, vault });
...
async function resolveWorkspaceSummary(
  ...
  try {
    return await akbGetWorkspaceSummary({ adapter, vault });
  } catch {
...
async function resolveIssueContext(
  ...
  try {
    const { issue, content } = await akbReadIssue({
