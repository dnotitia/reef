...
import { buildIssueUpdateMetadataPatch } from "@reef/core";
import { isForwardStatus, withRecoveredDraftStatus } from "@reef/core";
import type { ActivitySuggestion } from "@reef/core";
import {
  ...
  return tracer.startActiveSpan(
    ...
    async (span) => {
      ...
      try {
        ...
        const { suggestion } = await akbReadActivitySuggestion({
          adapter,
          vault,
          id,
        });
        ...
        if (suggestion.kind === "draft") {
          ...
          const existingIssue = (
            await akbListIssues({ adapter, vault })
          ).issues.find(
          ...
          const issueId = await akbAllocateNextIssueId({
            adapter,
            vault,
            prefix,
          });
        ...
        // Idempotent recovery: akbWriteIssue (issue) and the suggestion-status update
