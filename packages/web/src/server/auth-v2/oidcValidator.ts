...
function validateConfig(
  ...
  const providerType = config.providerType ?? "keycloak-oidc";
  if (
...
function parseValidatedClaims(
  ...
  const providerClaim = payload.identity_provider;
  ...
  if (
    config.providerType === "local-realm"
      ? providerClaim !== undefined
      : providerClaim !== config.providerAlias
  ) {
    throw new OidcTokenValidationError("oidc_token_invalid", "invalid");
  }
  ...
  if (
    !requiredStringClaim(payload.jti) ||
    !requiredStringClaim(payload.scope) ||
    expiresAt <= issuedAt
  ) {
    throw new OidcTokenValidationError("oidc_token_invalid", "invalid");
  }
...
function requiredNumericClaim(value: unknown): number {
  const parsed = optionalNumericClaim(value);
  if (parsed === undefined) {
    throw new OidcTokenValidationError("oidc_token_invalid", "invalid");
  }
  return parsed;
...
function requiredStringClaim(value: unknown): string {
  const parsed = optionalStringClaim(value);
  if (parsed === undefined) {
    throw new OidcTokenValidationError("oidc_token_invalid", "invalid");
  }
  return parsed;
