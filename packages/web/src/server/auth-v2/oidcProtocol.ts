...
/**
 * Reef-owned OIDC protocol for the current companion profile. It is deliberately
...
export function createAuthV2OidcProtocol(params: {
  ...
  contract: Extract<AkbAuthConfig, { auth_mode: "sso" }>;
  providerAlias: string;
  ...
  if (!provider || !PROVIDER_ALIAS_RE.test(params.providerAlias)) {
  ...
  const validator = createOidcTokenValidator({
    canonicalIssuer: params.runtime.issuer,
  ...
  try {
    ...
    if (
      ...
      (providerType === "local-realm"
        ? payload.identity_provider !== undefined
        : payload.identity_provider !== providerAlias) ||
      !nonceMatches ||
      payload.sub !== subject ||
      typeof payload.sid !== "string" ||
      payload.sid !== expectedSessionId ||
      (expectedNonce !== undefined && typeof payload.auth_time !== "number") ||
      !atHashMatches
    ) {
...
function callbackUri(runtime: AuthV2EnabledRuntimeConfig): string {
  return `${runtime.publicOrigin}/api/auth/akb/sso/callback`;
}
